...
export default function RecruiterRegisterPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const auth = await api<AuthResponse>("/api/v1/auth/register/recruiter", {
        method: "POST",
        body: { fullName, email, password, organizationName },
      });
      setAuth(auth);
  ...
  return (
    <AppShell>
      <div className="tp-container-narrow tp-fade-up">
        ...
        <h1 className="tp-display mt-2 text-[clamp(2rem,6vw,2.6rem)]">Create organization</h1>
        ...
        <form onSubmit={onSubmit} className="tp-panel mt-8 grid gap-4 p-5 sm:p-8">
          ...
          <label className="tp-field">
            <span className="tp-label">Work email</span>
            <input
              className="tp-input"
              type="email"
              required
          ...
          <label className="tp-field">
            <span className="tp-label">Organization</span>
            <input
              className="tp-input"
              required
